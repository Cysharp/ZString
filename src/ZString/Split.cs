...
namespace Cysharp.Text
{
    ...
    public readonly unsafe ref struct Split<T> where T : IEquatable<T>
    {
        private readonly ReadOnlySpan<T> _buffer;
        private readonly ReadOnlySpan<T> _separator;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Split(ReadOnlySpan<T> buffer, in T separator)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Split(ReadOnlySpan<T> buffer, ReadOnlySpan<T> separator)
        {
            if (Unsafe.AsPointer(ref MemoryMarshal.GetReference(buffer)) == null || buffer.Length == 0)
                throw new ArgumentNullException(nameof(buffer));
