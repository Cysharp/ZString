...
(U)     "U"                 Universal time with full                culture-specific                        Sunday, October 31, 1999 10:00:00 AM
    ...
    */
    ...
    class DateTimeFormat
    {
        ...
        //
        ////////////////////////////////////////////////////////////////////////////
        internal static void FormatDigits(ref Cysharp.Text.Utf16ValueStringBuilder outputBuffer, int value, int len)
        {
            Debug.Assert(value >= 0, "DateTimeFormat.FormatDigits(): value >= 0");
            FormatDigits(ref outputBuffer, value, len, false);
        }

        internal static unsafe void FormatDigits(ref Cysharp.Text.Utf16ValueStringBuilder outputBuffer, int value, int len, bool overrideLengthLimit)
        {
            Debug.Assert(value >= 0, "DateTimeFormat.FormatDigits(): value >= 0");
        ...
        //
        internal static int ParseQuoteString(ReadOnlySpan<char> format, int pos, ref Cysharp.Text.Utf16ValueStringBuilder result)
        {
            //
        ...
        //
        private static Cysharp.Text.Utf16ValueStringBuilder FormatCustomized(
            DateTime dateTime, ReadOnlySpan<char> format, DateTimeFormatInfo dtfi, TimeSpan offset, ref Cysharp.Text.Utf16ValueStringBuilder result)
        {
            ...
            if (result.IsEmpty())
            {
                ...
                result = Cysharp.Text.ZString.CreateStringBuilder();
            }
            ...
            while (i < format.Length)
            {
                ...
                switch (ch)
                {
                    ...
                    case 'F':
                        ...
                        {
                            long fraction = (dateTime.Ticks % DateTimeEx.TicksPerSecond);
                            ...
                            if (ch == 'f')
                            {
                                ...
                                result.AppendFormat(fixedNumberFormats[tokenLen - 1], (int)fraction);
                            }
                            else
                            {
                                ...
                                if (effectiveDigits > 0)
                                {
                                    ...
                                    result.AppendFormat(fixedNumberFormats[effectiveDigits - 1], (int)fraction);
                                }
                                else
                                {
                    ...
                    case 'M':
                        ...
                        {
                            bool gentivie = IsUseGenitiveForm(format, i, tokenLen, 'd');

                            if (gentivie && tokenLen >= 4)
                            {
                                //TODO: If you use a genitive month, then heap allocation will occur.
                                string[] monthNamesArray = dtfi.MonthGenitiveNames;
                                if (month < 1 || month > monthNamesArray.Length)
                                    throw new ArgumentOutOfRangeException(nameof(month));
                                result.Append(monthNamesArray[month - 1]);
                            }
                            else
                            {
                    ...
                    case '%':
                        ...
                        {
                            ReadOnlySpan<char> nextCharCharFormat = stackalloc char[]{(char)nextChar};
                            var origStringBuilder = FormatCustomized(dateTime, nextCharCharFormat, dtfi, offset, ref result);
        ...
        private static void FormatCustomizedTimeZone(DateTime dateTime, TimeSpan offset, ReadOnlySpan<char> format, int tokenLen, bool timeOnly, ref Cysharp.Text.Utf16ValueStringBuilder result)
        {
            ...
            if (dateTimeFormat)
            {
                ...
                if (timeOnly && dateTime.Ticks < DateTimeEx.TicksPerDay)
                {
        ...
        private static void FormatCustomizedRoundripTimeZone(DateTime dateTime, TimeSpan offset, ref Cysharp.Text.Utf16ValueStringBuilder result)
        {
        ...
        private static void Append2DigitNumber(ref Cysharp.Text.Utf16ValueStringBuilder result, int val)
        {
        ...
        internal static string GetRealFormat(ReadOnlySpan<char> format, DateTimeFormatInfo dtfi)
        {
            ...
            switch (format[0])
            {
                ...
                case 'g':       // General (short date + short time)
                    realFormat = GetGeneralShortTimePatternCache(dtfi);
                    break;
                case 'G':       // General (short date + long time)
                    realFormat = GetGeneralLongTimePatternCache(dtfi);
                    break;
                case 'm':
        ...
        internal static bool TryFormat(DateTime dateTime, Span<char> destination, out int charsWritten, ReadOnlySpan<char> format, IFormatProvider provider, TimeSpan offset)
        {
            ...
            {
                //TODO: Currently, only the Gregorian calendar is supported.
                charsWritten = 0;
                return false;
            }

            var sb = FormatStringBuilder(dateTime, format, dtfi, offset);
            try
            {
        ...
        private static Cysharp.Text.Utf16ValueStringBuilder FormatStringBuilder(DateTime dateTime, ReadOnlySpan<char> format, DateTimeFormatInfo dtfi, TimeSpan offset)
        {
            Debug.Assert(dtfi != null);
            if (format.Length == 0)
            {
                bool timeOnlySpecialCase = false;
                if (dateTime.Ticks < DateTimeEx.TicksPerDay)
                {
                    if (!(dtfi.Calendar is GregorianCalendar))
                    {
                        throw new NotSupportedException("TODO: Currently, only the Gregorian calendar is supported.");
                    }
                }
                if (offset == NullOffset)
                {
                ...
                else
                {
                    ...
                    format = (timeOnlySpecialCase ? RoundtripDateTimeUnfixed : GetDateTimeOffsetPatternCache(dtfi)).AsSpan();
                }
            }

            if (format.Length == 1)
            {
                format = ExpandPredefinedFormat(format, ref dateTime, ref dtfi, ref offset).AsSpan();
            }
            var stringBuilder = Cysharp.Text.ZString.CreateStringBuilder();
            return FormatCustomized(dateTime, format, dtfi, offset, ref stringBuilder);
        }
