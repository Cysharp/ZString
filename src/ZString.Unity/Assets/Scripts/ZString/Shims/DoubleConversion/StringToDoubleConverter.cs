...
#pragma warning restore 660
    ...
    internal static partial class StringToDoubleConverter
    {
        ...
        static readonly byte[] infinity_symbol_ = Encoding.UTF8.GetBytes(double.PositiveInfinity.ToString());
        static readonly byte[] nan_symbol_ = Encoding.UTF8.GetBytes(double.NaN.ToString());
                    ...
                    out int processed_characters_count)
        {
            ...
            if (converted == null)
            {
                ...
                if (fallbackBuffer.Length < processed_characters_count)
                {
                    Array.Resize(ref fallbackBuffer, processed_characters_count);
                }
                var fallbackI = 0;
                while (input != current)
                {
