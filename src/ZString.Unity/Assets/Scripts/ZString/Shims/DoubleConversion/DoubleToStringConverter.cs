...
namespace Cysharp.Text.Internal
{
    ...
    internal ref struct StringBuilder
    {
        ...
        public bool TryAddCharacter(char str)
        {
        ...
        public bool TryAddCharacter(byte str)
        {
            if (buffer.Length - offset < 1) return false;
            buffer[offset++] = (char)str;
            return true;
        }

        public bool TryAddString(char[] str)
        {
        ...
        public bool TryAddSubstring(char[] str, int length)
        {
        ...
        public bool TryAddSubstring(byte[] str, int length)
        {
            if (buffer.Length - offset < length) return false;
            for (int i = 0; i < length; i++)
            {
                buffer[offset + i] = (char)str[i];
            }
            offset += length;
            return true;
        }

        public bool TryAddSubstring(char[] str, int start, int length)
        {
            if (buffer.Length - offset < (length - start)) return false;
            for (int i = 0; i < length; i++)
            {
        ...
        public bool TryAddSubstring(byte[] str, int start, int length)
        {
            if (buffer.Length - offset < (length - start)) return false;
            for (int i = 0; i < length; i++)
            {
                buffer[offset + i] = (char)str[start + i];
            }
            offset += length;
            return true;
        }

        public bool TryAddPadding(char c, int count)
        {
        ...
        public bool TryAddStringSlow(string str)
        {
            var s = str.AsSpan();
            if (s.TryCopyTo(buffer.Slice(offset)))
            {
                offset += s.Length;
                return true;
            }
    ...
    {
        public static bool TryFormat(Span<char> destination, float value, out int charsWritten)
        {
            ...
            charsWritten = sb.offset;
            return true;
        }

        public static bool TryFormat(Span<char> destination, double value, out int charsWritten)
        {
            var sb = new StringBuilder(destination);
    ...
    internal static partial class DoubleToStringConverter
    {
        ...
        // C# constants
        static readonly char[] infinity_symbol_ = double.PositiveInfinity.ToString().ToCharArray();
        static readonly char[] nan_symbol_ = double.NaN.ToString().ToCharArray();
            ...
            ref StringBuilder result_builder)
        {
            ...
            if (double_inspect.IsInfinite())
            {
                ...
                if (value < 0)
                {
                    if (!result_builder.TryAddCharacter('-')) return false;
                }
                if (!result_builder.TryAddString(infinity_symbol_)) return false;
                return true;
            }
            if (double_inspect.IsNan())
            {
                if (nan_symbol_ == null) return false;
                if (!result_builder.TryAddString(nan_symbol_)) return false;
                return true;
            }
            return false;
        }
            ...
            DtoaMode mode)
        {
            ...
            if (!fastworked)
            {
                ...
                if (!result_builder.TryAddStringSlow(str)) return false;
                return true;
            }
            ...
            {
                return CreateDecimalRepresentation(decimal_rep, decimal_rep_length,
                                            decimal_point,
            ...
            {
                return CreateExponentialRepresentation(decimal_rep, decimal_rep_length, exponent,
                                                ref result_builder);
            }
        }

        static bool CreateDecimalRepresentation(
            byte[] decimal_digits,
            int length,
            int decimal_point,
            ...
            ref StringBuilder result_builder)
        {
            ...
            if (decimal_point <= 0)
            {
                ...
                if (!result_builder.TryAddCharacter('0')) return false;
                if (digits_after_point > 0)
                {
                    if (!result_builder.TryAddCharacter('.')) return false;
                    if (!result_builder.TryAddPadding('0', -decimal_point)) return false;
                    if (!result_builder.TryAddSubstring(decimal_digits, length)) return false;
                    int remaining_digits = digits_after_point - (-decimal_point) - length;
                    if (!result_builder.TryAddPadding('0', remaining_digits)) return false;
                }
            }
            else if (decimal_point >= length)
            {
                ...
                if (!result_builder.TryAddSubstring(decimal_digits, length)) return false;
                if (!result_builder.TryAddPadding('0', decimal_point - length)) return false;
                if (digits_after_point > 0)
                {
                    if (!result_builder.TryAddCharacter('.')) return false;
                    if (!result_builder.TryAddPadding('0', digits_after_point)) return false;
                }
            }
            else
            {
                // "decima.l_rep000".
                if (!result_builder.TryAddSubstring(decimal_digits, decimal_point)) return false;
                if (!result_builder.TryAddCharacter('.')) return false;
                if (!result_builder.TryAddSubstring(decimal_digits, decimal_point, length - decimal_point)) return false;
        ...
        static bool CreateExponentialRepresentation(
            byte[] decimal_digits,
            int length,
            int exponent,
            ref StringBuilder result_builder)
        {
            if (!result_builder.TryAddCharacter(decimal_digits[0])) return false;
            if (length != 1)
            {
                if (!result_builder.TryAddCharacter('.')) return false;
                if (!result_builder.TryAddSubstring(decimal_digits, 1, length - 1)) return false;
            }
            if (!result_builder.TryAddCharacter(exponent_character_)) return false;
            if (exponent < 0)
            {
                if (!result_builder.TryAddCharacter('-')) return false;
                exponent = -exponent;
            ...
            if (!result_builder.TryAddSubstring(buffer, first_char_pos, kMaxExponentLength - first_char_pos)) return false;
